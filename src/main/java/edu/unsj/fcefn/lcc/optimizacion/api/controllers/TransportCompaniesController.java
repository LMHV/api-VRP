...
// Se encarga de recibir los request's
...
public class TransportCompaniesController {
    ...
    @GetMapping(value = "{id}")
    public TransportCompanyDTO find(@PathVariable("id") Integer id) { return transportCompaniesService.find(id); }

    @PostMapping(value = "")
    public TransportCompanyDTO add(@RequestBody TransportCompanyDTO transportCompanyDTO){
        // El requestBody de arriba lo que hace es vincular el body que viene en el JSON enviado por el POST al
        // objeto creado transportCompanyDTO
